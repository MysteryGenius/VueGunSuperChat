    ...
    <div class="max-w-md w-full space-y-8">
      ...
      <div class="mt-8 space-y-6">
        ...
        <div class="flex space-x-2">
          <button @click="signup()" type="submit" class="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500">
            Sign up
          </button>
...
<script>
  ...
  export default {
    ...
    methods: {
      login() {
        user.auth(this.username, this.password, ({ err }) => err && alert(err));
      },
      signup() {
        user.create(this.username, this.password, ({ err }) => {
          if (err) {
            alert(err);
          } else {
            this.login();
